...
NODE_SIZES = {
    ...
    "standard": (16, 16, 0),
    "llm": (
        # Single-H100 local LLM default. Override via env vars if a different
        # model or host envelope needs a smaller or larger guest shape.
        _env_int("EASYENCLAVE_LLM_MEM_GIB", 160),
        _env_int("EASYENCLAVE_LLM_VCPUS", 32),
        _env_int("EASYENCLAVE_LLM_DISK_GIB", 300),
    ),
}
...
def _normalize_pci_bdf(value: str) -> str:
    """Normalize PCI BDF to 0000:bb:ss.f lowercase form."""
    bdf = str(value or "").strip().lower()
    if bdf and bdf.count(":") == 1:
        bdf = f"0000:{bdf}"
    if not PCI_BDF_RE.fullmatch(bdf):
        raise ValueError(
            f"Invalid PCI BDF {value!r}. Expected 0000:bb:ss.f or bb:ss.f (e.g. 0000:0d:00.0)"
        )
    return bdf


def _parse_hostdev_pci_args(values: list[str] | None) -> list[str]:
    """Parse repeatable/comma-separated --hostdev-pci argument values."""
    parsed: list[str] = []
    seen: set[str] = set()
...
class TDXManager:
    ...
    def vm_new(
        ...
        size_name: str = "",
        disk_gib: int = 0,
        hostdev_pci: list[str] | None = None,
    ) -> dict:
        ...
        Args:
            ...
            hostdev_pci: Host PCI devices to passthrough via VFIO (e.g. ["0000:0d:00.0"])
        ...
        hostdev_devices_xml = self._build_hostdev_devices_xml(hostdev_pci)
        xml_content = xml_content.replace("HOSTDEV_DEVICES", hostdev_devices_xml)
        xml_content = xml_content.replace("EXTRA_QEMU_ARGS", self._build_extra_qemu_args(hostdev_pci))
        ...
        # Large TDX VMs and some hostdev passthrough configurations can fail with
        # the default proportional SWIOTLB size. Pin to a fixed 512MB buffer.
        if memory_gib >= 64 or bool(hostdev_pci):
    ...
    def _build_hostdev_devices_xml(self, hostdev_pci: list[str] | None) -> str:
        """Render <hostdev> blocks for VFIO PCI passthrough."""
        devices = hostdev_pci or []
        if not devices:
            return ""

        chunks: list[str] = []
        for raw_bdf in devices:
            bdf = _normalize_pci_bdf(raw_bdf)
            dev_path = Path("/sys/bus/pci/devices") / bdf
            if not dev_path.exists():
                raise FileNotFoundError(f"PCI device not found on host: {bdf}")

            driver_name = ""
            driver_link = dev_path / "driver"
            if driver_link.exists():
                try:
                    driver_name = Path(os.path.realpath(driver_link)).name
                except OSError:
                    driver_name = ""
            if driver_name and driver_name != "vfio-pci":
                raise RuntimeError(
                    f"PCI device {bdf} is bound to {driver_name!r}, expected 'vfio-pci' before passthrough"
                )

            domain, bus, slot_func = bdf.split(":", 2)
    ...
    def _build_extra_qemu_args(self, hostdev_pci: list[str] | None) -> str:
        """Render additional QEMU args needed when passthrough devices are attached."""
        if not (hostdev_pci or []):
            return ""
        # H100-class GPUs expose very large BARs and need a larger 64-bit MMIO aperture.
        hole64_size = (os.environ.get("EASYENCLAVE_PCI_HOLE64_SIZE") or "128G").strip() or "128G"
        return "\n".join(
            [
                "    <qemu:arg value='-global'/>",
                f"    <qemu:arg value='q35-pcihost.pci-hole64-size={hole64_size}'/>",
            ]
        )
...
To start a new EasyEnclave network:
  ...
  3. POST /api/v1/deployments         Deploy workloads via API
    ...
    new_parser.add_argument(
        ...
        default=[],
        help=(
            "Host PCI BDF(s) to passthrough via VFIO (repeatable or comma-separated), "
            "e.g. --hostdev-pci 0000:0d:00.0"
        ),
    )
    ...
    try:
        if args.command == "control-plane":
            ...
            if args.vm_command == "new":
                hostdev_pci = _parse_hostdev_pci_args(args.hostdev_pci)
                config = {
                ...
                if hostdev_pci:
                    print(
                        "Attaching host PCI devices: " + ", ".join(hostdev_pci),
                        file=sys.stderr,
                    )
                result = mgr.vm_new(
                    args.image,
