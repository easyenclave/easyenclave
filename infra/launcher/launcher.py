...
services:
  easyenclave:
    ...
    environment:
      ...
      - EE_GCP_IMAGE_PROJECT=${{EE_GCP_IMAGE_PROJECT:-}}
      - EE_GCP_IMAGE_FAMILY=${{EE_GCP_IMAGE_FAMILY:-}}
      - EE_GCP_IMAGE_NAME=${{EE_GCP_IMAGE_NAME:-}}
...
def push_heartbeat_to_control_plane(
    *,
    ...
    logger.info(
        "Skipping legacy heartbeat push for agent_id=%s vm_name=%s status=%s deployment_id=%s",
        agent_id,
        vm_name,
        status,
        deployment_id,
...
def start_periodic_attestation_push(*, agent_id: str, vm_name: str, config: dict):
    ...
    logger.info(
        "Periodic attestation push disabled for agent_id=%s vm_name=%s; current control plane has no heartbeat ingest endpoint",
        agent_id,
        vm_name,
    )
    return None
...
class AgentAPIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _notify_deployment_complete(self, deployment_id: str, attestation: dict, config: dict):
        ...
        logger.info(
            "Skipping legacy deployment-complete callback for deployment_id=%s agent_id=%s",
            deployment_id,
            _admin_state.get("agent_id"),
        )

    def _notify_deployment_failed(self, deployment_id: str, error: str):
        """Notify control plane that deployment failed."""
        logger.warning(
            "Skipping legacy deployment-failed callback for deployment_id=%s agent_id=%s error=%s",
            deployment_id,
            _admin_state.get("agent_id"),
            error,
        )
...
def _handoff_to_ee_agent(config: dict, vm_name: str) -> bool:
    """Exec the current Rust agent when it is bundled into the image."""
    handoff_enabled = str(
        config.get("enable_ee_agent_handoff")
        or os.environ.get("EASYENCLAVE_ENABLE_EE_AGENT_HANDOFF", "")
    ).strip().lower() in {"1", "true", "yes", "on"}
    if not handoff_enabled:
        logger.info("ee-agent handoff disabled; using Python launcher agent")
        return False

    ee_agent_bin = Path("/usr/local/bin/ee-agent")
    if not (ee_agent_bin.is_file() and os.access(ee_agent_bin, os.X_OK)):
        return False

    node_size = str(config.get("node_size") or "").strip().lower() or "tiny"
    datacenter = resolve_datacenter_label(config)
    cp_url = str(config.get("control_plane_url") or CONTROL_PLANE_URL).strip()
    if not cp_url:
        raise RuntimeError("control_plane_url is required for ee-agent handoff")
    ...
    intel_api_key = str(config.get("intel_api_key") or "").strip()
    if intel_api_key:
        agent_config["intel_api_key"] = intel_api_key

    # The guest rootfs is dm-verity protected and mounted read-only, so write
    # the runtime config into tmpfs before handing off to the Rust agent.
    config_path = Path("/run/easyenclave/agent.json")
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(json.dumps(agent_config, indent=2) + "\n", encoding="utf-8")
    config_path.chmod(0o600)

    env = os.environ.copy()
    env["EE_AGENT_MODE"] = "agent"
    env["EASYENCLAVE_CONFIG"] = str(config_path)
    env["VM_NAME"] = vm_name

    logger.info(
        "Handing off to ee-agent: vm_name=%s cp_url=%s node_size=%s datacenter=%s",
        vm_name,
        cp_url,
        node_size,
        datacenter,
    )
    os.execve(str(ee_agent_bin), [str(ee_agent_bin)], env)
...
def request_nonce_challenge(vm_name: str) -> str:
    ...
    try:
        response = requests.get(
            f"{CONTROL_PLANE_URL}/api/agents/challenge",
            timeout=30,
        )
        response.raise_for_status()
        ...
        nonce = result["nonce"]
        ttl_seconds = result.get("expires_in_seconds", 300)
...
def register_with_control_plane(
    ...
    tdx = attestation.get("tdx") or {}
    intel_ta_token = str(tdx.get("intel_ta_token") or "").strip()
    if not intel_ta_token:
        raise RuntimeError("Initial attestation missing intel_ta_token for registration")

    nonce = str(attestation.get("nonce") or "").strip()
    if not nonce:
        raise RuntimeError("Initial attestation missing nonce for registration")
...
def _cloudflared_runtime_paths() -> list[Path]:
    """Return preferred cloudflared install paths (most to least durable)."""
    return [
        Path("/usr/local/bin/cloudflared"),
        Path("/run/easyenclave/bin/cloudflared"),
        Path("/var/tmp/easyenclave/bin/cloudflared"),
        Path("/tmp/easyenclave/bin/cloudflared"),
    ]


def _find_cloudflared_binary() -> str | None:
    """Resolve an executable cloudflared path from PATH or runtime locations."""
    which_path = shutil.which("cloudflared")
    if which_path:
        return which_path

    for candidate in _cloudflared_runtime_paths():
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return str(candidate)
    return None


def _cloudflared_is_runnable(binary: str) -> bool:
    """Validate cloudflared binary execution and emit version metadata."""
    try:
        out = subprocess.run([binary, "--version"], capture_output=True, text=True, timeout=10)
        if out.returncode != 0:
            logger.warning(
                "cloudflared candidate not runnable (%s): rc=%s stderr=%s",
                binary,
                out.returncode,
                (out.stderr or "").strip(),
            )
            return False
        logger.info("cloudflared available at %s: %s", binary, (out.stdout or "").strip())
        return True
    except Exception as exc:
        logger.warning("cloudflared candidate execution failed (%s): %s", binary, exc)
        return False


def _ensure_cloudflared_installed() -> str | None:
    ...
    """
    binary = _find_cloudflared_binary()
    if binary and _cloudflared_is_runnable(binary):
        return binary
    ...
    for dest in _cloudflared_runtime_paths():
        try:
            dest.parent.mkdir(parents=True, exist_ok=True)
            subprocess.run(["curl", "-fsSL", url, "-o", str(dest)], check=True, timeout=60)
            subprocess.run(["chmod", "+x", str(dest)], check=True)
            if dest.is_file() and os.access(dest, os.X_OK) and _cloudflared_is_runnable(str(dest)):
                return str(dest)
        except Exception as exc:
            last_error = exc
            logger.warning("cloudflared install attempt failed at %s: %s", dest, exc)
    ...
    if last_error is not None:
        logger.warning("cloudflared runtime install failed: %s", last_error)
    return None
...
def start_cloudflared(tunnel_token: str) -> subprocess.Popen | None:
    ...
    cloudflared_bin = _ensure_cloudflared_installed()
...
def run_agent_mode(config: dict):
    ...
    if _handoff_to_ee_agent(config, vm_name):
        return
    ...
    if max_registration_attempts > 0:
        logger.info(
            "Agent registration retry policy: max_attempts=%s base=%ss max=%ss stop_vm_on_exhausted=true",
        ...
        logger.info(
            "Agent registration retry policy: unlimited attempts base=%ss max=%ss stop_vm_on_exhausted=true",
            registration_retry_base_seconds,
            registration_retry_max_seconds,
        )
