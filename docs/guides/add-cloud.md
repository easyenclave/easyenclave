...
Primary directories: `src/`, `infra/`, `.github/workflows/`
...
- Add provider client/service under `src/cp/services/`.
...
- Add provider config fields in the Ansible orchestration layer under `ansible/playbooks/`.
- Export provider env vars in `src/bin/ee-agent`.
