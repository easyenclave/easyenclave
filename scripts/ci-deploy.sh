...
# Keep host-side bootstrap traffic on the internal URL for determinism.
# Agent VMs may not always have direct guest->guest reachability to the control-plane VM,
# so agent registration should prefer the public URL when it is already reachable.
